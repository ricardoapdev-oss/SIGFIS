...
function InfoTooltip({ text }: { text: string }) {
  return (
    <div className="relative group inline-flex items-center shrink-0">
      ...
      <div className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 bg-zinc-800 border border-zinc-700 rounded-xl p-3 text-[10px] text-zinc-300 leading-relaxed shadow-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-50 text-left font-normal whitespace-normal">
        {text}
      </div>
    </div>
  );
...
function InsightsSection({ insights, onNavigate }: { insights: any[]; onNavigate: Props['onNavigate'] }) {
  ...
  if (!insights.length) return (
    <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-10 text-center">
      ...
      <p className="text-xs text-zinc-500 mt-1">A carteira está dentro dos parâmetros normais.</p>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-center">
          ...
          <p className="text-[9px] uppercase tracking-widest text-zinc-500 mt-0.5">Críticos</p>
        </div>
        <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 text-center">
          <p className="text-2xl font-black text-orange-400">{high}</p>
          <p className="text-[9px] uppercase tracking-widest text-zinc-500 mt-0.5">Altos</p>
        </div>
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 text-center">
          <p className="text-2xl font-black text-amber-400">{medium}</p>
          <p className="text-[9px] uppercase tracking-widest text-zinc-500 mt-0.5">Médios</p>
        </div>
      </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    <span className="text-[9px] text-zinc-500 uppercase tracking-widest">{catLabel[ins.category] ?? ins.category}</span>
                    {ins.contractNumber && <span className="text-[9px] text-zinc-600">{ins.contractNumber}</span>}
                  </div>
                  <p className="text-xs font-bold text-zinc-200">{ins.title}</p>
                  <p className="text-[11px] text-zinc-400 mt-1 leading-snug">{ins.description}</p>
                  <div className="mt-2.5 flex items-center justify-between gap-2">
                    <div className="flex-1 bg-zinc-900/60 border border-zinc-800/60 rounded-lg px-2.5 py-1.5">
                      <p className="text-[9px] text-zinc-500 uppercase tracking-widest mb-0.5">Ação Recomendada</p>
                      <p className="text-[11px] text-zinc-300">{ins.action}</p>
                    </div>
                    {ins.contractId && (
...
function RecommendationsSection({ recommendations, onNavigate }: { recommendations: any[]; onNavigate: Props['onNavigate'] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-[10px] text-zinc-500 uppercase tracking-widest">{recommendations.length} ações priorizadas por impacto</p>
      {recommendations.map((rec, idx) => (
        <div key={idx} className="bg-zinc-900/30 border border-zinc-900 rounded-xl p-4 flex items-start gap-4">
          <div className="h-7 w-7 rounded-lg bg-zinc-800 flex items-center justify-center text-xs font-black text-zinc-400 shrink-0">{rec.priority}</div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">{rec.category}</span>
              ...
              <span className={`text-[8px] font-bold uppercase px-1.5 py-0.5 rounded ${rec.effort === 'LOW' ? 'text-emerald-400 bg-emerald-500/10' : rec.effort === 'MEDIUM' ? 'text-zinc-400 bg-zinc-800' : 'text-red-400 bg-red-500/10'}`}>
                Esforço {rec.effort === 'LOW' ? 'Baixo' : rec.effort === 'MEDIUM' ? 'Médio' : 'Alto'}
              </span>
            </div>
            <p className="text-xs font-bold text-zinc-200">{rec.title}</p>
          ...
          {rec.contractId && (
            <button onClick={() => onNavigate('details', rec.contractId)}
              className="shrink-0 flex items-center gap-1 text-[9px] font-bold text-zinc-400 hover:text-white border border-zinc-800 hover:border-zinc-700 px-2.5 py-1.5 rounded-lg transition-colors cursor-pointer">
              Ver <ChevronRight className="h-2.5 w-2.5" />
            </button>
          )}
        </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {predictions.map((pred, i) => {
          ...
          const trendColor = pred.trend === 'UP' ? 'text-emerald-400' : pred.trend === 'DOWN' ? 'text-red-400' : 'text-zinc-400';
          const barColor = pred.probability > 0.7 ? '#ef4444' : pred.probability > 0.4 ? '#f59e0b' : '#10b981';
          return (
            <div key={i} className="bg-zinc-900/30 border border-zinc-900 rounded-xl p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <TrendIcon className={`h-3.5 w-3.5 ${trendColor}`} />
                    <span className="text-[9px] text-zinc-500 uppercase tracking-widest">{pred.timeframe}</span>
                  </div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <p className="text-xs font-bold text-zinc-200">{pred.title}</p>
                    {(pred as any).tooltip && <InfoTooltip text={(pred as any).tooltip} />}
                  </div>
                  <p className="text-[11px] text-zinc-400 leading-snug">{pred.description}</p>
                </div>
                <div className="shrink-0 text-right">
                  <p className="text-lg font-black text-zinc-200">{Math.round(pred.probability * 100)}%</p>
                  <p className="text-[9px] text-zinc-500 uppercase tracking-widest">Probabilidade</p>
                  <div className="mt-1.5 h-1.5 w-16 bg-zinc-800 rounded-full overflow-hidden">
        ...
        return (
          <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-5">
            ...
            <div className="flex items-center gap-4 mt-2">
              <div className="flex items-center gap-1.5"><div className="h-1.5 w-4 rounded-full bg-indigo-500" /><span className="text-[9px] text-zinc-500">Carteira ativa (R$ M — eixo esquerdo)</span></div>
              <div className="flex items-center gap-1.5"><div className="h-0.5 w-4 rounded-full bg-emerald-500" /><span className="text-[9px] text-zinc-500">% executado (eixo direito)</span></div>
            </div>
          </div>
        );
      })()}
...
function ComplianceSection({ compliance }: { compliance: any }) {
  if (!compliance) return <div className="text-center py-8 text-zinc-500 text-xs">Dados insuficientes para calcular compliance.</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-5 flex flex-col sm:flex-row items-center gap-6">
        <div className="text-center shrink-0">
          <ScoreGauge score={overallScore} />
          <div className="mt-2 flex items-center justify-center gap-2">
            <GradeChip grade={grade} />
            <div className="text-left">
              <p className="text-xs font-bold text-zinc-300">Score Geral</p>
              <p className="text-[10px] text-zinc-500">Compliance Contratual</p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-zinc-900">
          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Auditoria por Contrato (Top 10 Ativos)</p>
        </div>
          ...
          {contractsAudit.map((c: any) => {
            ...
            return (
              <div key={c.contractId} className="px-4 py-3 flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-zinc-300">{c.contractNumber}</p>
                  <p className="text-[10px] text-zinc-500 truncate">{c.object}</p>
                  {c.issues.length > 0 && (
                ...
                <div className="shrink-0 text-right">
                  <p className="text-sm font-bold text-zinc-300">{c.score}</p>
...
function SuppliersSection({ suppliers }: { suppliers: any[] }) {
  if (!suppliers.length) return <div className="text-center py-8 text-zinc-500 text-xs">Nenhum fornecedor encontrado.</div>;
  ...
  return (
    <div className="space-y-4">
        ...
        {([
          ...
          <div key={grade} className={`${color} border rounded-xl p-3 text-center`}>
            ...
            <div className="flex items-center justify-center gap-1 mt-0.5">
              <p className="text-[9px] text-zinc-500 uppercase tracking-widest">Nota {grade}</p>
      ...
      <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl overflow-hidden">
        <div className="px-4 py-3 grid grid-cols-12 gap-2 text-[9px] font-bold text-zinc-600 uppercase tracking-widest border-b border-zinc-900">
        ...
        <div className="divide-y divide-zinc-900/40">
          {suppliers.slice(0, 12).map((s, i) => (
            <div key={s.contractorId} className="px-4 py-3 grid grid-cols-12 gap-2 items-center hover:bg-zinc-900/20 transition-colors">
              <div className="col-span-1 text-xs font-bold text-zinc-600">{i + 1}</div>
              <div className="col-span-4 min-w-0">
                <p className="text-xs font-bold text-zinc-200 truncate">{s.name}</p>
                <p className="text-[9px] text-zinc-500 truncate">{s.category}</p>
              </div>
              <div className="col-span-2 text-center">
                <p className="text-xs font-bold text-zinc-300">{s.activeContracts}</p>
                <p className="text-[9px] text-zinc-600">ativos</p>
              </div>
              <div className="col-span-2 text-center">
                <p className={`text-xs font-bold ${s.openOccurrences > 0 ? 'text-red-400' : 'text-zinc-400'}`}>{s.openOccurrences}</p>
                <p className="text-[9px] text-zinc-600">abertas</p>
              </div>
              <div className="col-span-2 text-center">
                <p className="text-xs font-bold text-zinc-200">{s.score}</p>
  ...
  return (
    <div className="space-y-4">
      <div className="bg-violet-500/5 border border-violet-500/20 rounded-xl p-3 flex items-start gap-2">
        ...
        <p className="text-[11px] text-zinc-400">Assistente baseado nos dados reais da carteira. Faça perguntas em linguagem natural sobre contratos, fiscais, medições e valores.</p>
      </div>
      <div className="bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 min-h-[240px] max-h-[400px] overflow-y-auto space-y-3">
        {chatHistory.length === 0 ? (
          <div className="text-center py-8">
            <MessageSquare className="h-8 w-8 text-zinc-700 mx-auto mb-2" />
            <p className="text-xs text-zinc-600">Faça uma pergunta sobre a carteira de contratos</p>
            <div className="flex flex-wrap gap-2 justify-center mt-4">
              {suggestions.slice(0, 3).map((s, i) => (
                <button key={i} onClick={() => setChatInput(s)}
                  className="text-[10px] text-zinc-400 hover:text-white border border-zinc-800 hover:border-zinc-700 px-2.5 py-1 rounded-lg transition-colors cursor-pointer">
                  {s}
                </button>
              ))}
            </div>
          ...
          chatHistory.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-xl px-3 py-2 ${msg.role === 'user' ? 'bg-violet-500/20 border border-violet-500/30 text-zinc-200' : 'bg-zinc-900/60 border border-zinc-800 text-zinc-300'}`}>
      ...
      <div className="flex gap-2">
        <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()}
          ...
          className="flex-1 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-violet-500/50" />
      ...
      <div>
        <p className="text-[9px] text-zinc-600 uppercase tracking-widest mb-2">Sugestões rápidas</p>
        <div className="flex flex-wrap gap-2">
          {suggestions.map((s, i) => (
            <button key={i} onClick={() => setChatInput(s)}
              className="text-[10px] text-zinc-500 hover:text-zinc-300 border border-zinc-900 hover:border-zinc-800 px-2.5 py-1 rounded-lg transition-colors cursor-pointer">
              {s}
            </button>
          ))}
        </div>
...
function DiagnosisSection({ contracts, selectedContractId, setSelectedContractId, diagnosis, onNavigate }: {
  ...
  return (
    <div className="space-y-4">
      <div>
        <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Selecionar Contrato para Diagnóstico</p>
        <select value={selectedContractId} onChange={e => setSelectedContractId(e.target.value)}
          className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-violet-500/50 cursor-pointer">
      ...
      {!diagnosis ? (
        <div className="bg-zinc-900/20 border border-zinc-900 rounded-xl p-10 text-center">
          <Search className="h-8 w-8 text-zinc-700 mx-auto mb-3" />
          <p className="text-xs text-zinc-600">Selecione um contrato para gerar o diagnóstico de saúde</p>
        </div>
      ) : (() => {
          ...
          <div className="space-y-4">
            <div className="bg-zinc-900/30 border border-zinc-900 rounded-xl p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Diagnóstico Individual</p>
                  <p className="text-sm font-black text-white mt-0.5">{c.contractNumber}</p>
                  <p className="text-xs text-zinc-400 mt-0.5 line-clamp-2">{c.objectDescription}</p>
                  <div className="flex flex-wrap gap-4 mt-3 text-[10px]">
                    <div><p className="text-zinc-600">Contratada</p><p className="text-zinc-300 font-bold">{c.contractor?.tradeName || c.contractor?.name || '—'}</p></div>
                    <div><p className="text-zinc-600">Valor</p><p className="text-zinc-300 font-bold">{(c.currentValue ?? 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></div>
                    <div><p className="text-zinc-600">Vencimento</p><p className={`font-bold ${daysRemaining <= 30 ? 'text-red-400' : daysRemaining <= 90 ? 'text-amber-400' : 'text-zinc-300'}`}>{new Date(c.endDate).toLocaleDateString('pt-BR')} ({daysRemaining}d)</p></div>
                    <div><p className="text-zinc-600">Fiscal</p><p className={`font-bold ${fiscal ? 'text-zinc-300' : 'text-red-400'}`}>{fiscal?.name ?? 'Sem designação'}</p></div>
                  </div>
                </div>
              </div>
            </div>
...
export function AIInsightsPanel({ user, onNavigate }: Props) {
  ...
  const { data: contractsRaw, isLoading: contractsLoading } = useQuery({
    queryKey: ['intelligence-contracts', user.id],
  ...
  const autoInsights = useMemo(() => {
    ...
    const noFiscal = contracts.filter(c => c.status === 'ACTIVE' && !c.fiscalAssignments?.length);
    ...
    const pendingMsr = contracts.filter(c => c.hasPendingMeasurements);
    if (pendingMsr.length) insights.push({ id: 'pending-msr', category: 'MEASUREMENT', severity: 'MEDIUM', title: `${pendingMsr.length} medição(ões) aguardando homologação`, description: `Medições submetidas aguardando aprovação do gestor.`, action: 'Revisar e aprovar ou reprovar as medições pendentes.' });
    const openOcc = contracts.filter(c => c.hasOpenOccurrences);
  ...
  const recommendations = useMemo(() => {
    ...
    contracts.filter(c => c.hasPendingMeasurements)
    ...
    contracts.filter(c => c.hasOpenOccurrences)
  ...
  const processQuestion = useMemo(() => (q: string): string => {
    ...
    return `Não encontrei uma resposta específica. Tente:\n• "Quantos contratos vencem este mês?"\n• "Qual fiscal tem mais contratos?"\n• "Quais contratos têm medição pendente?"\n• "Qual o valor total da carteira?"\n• "Quais contratos estão sem fiscal?"`;
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center h-48 gap-3">
      ...
      <p className="text-xs text-zinc-500">Carregando inteligência contratual...</p>
    </div>
  );
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-[11px] text-zinc-500 mt-0.5">Análise preditiva, compliance e assistência baseada nos dados reais da carteira</p>
        </div>
        <button onClick={() => refetch()} className="flex items-center gap-1.5 text-[10px] text-zinc-500 hover:text-zinc-300 border border-zinc-800 bg-zinc-900/40 px-3 py-1.5 rounded-lg transition-colors cursor-pointer">
          <RefreshCw className="h-3 w-3" /> Atualizar
        </button>
      </div>

      <div className="flex gap-1 flex-wrap bg-zinc-900/30 p-1 rounded-xl border border-zinc-900">
        {SECTIONS.map(s => {
          const Icon = s.icon;
          return (
            <button key={s.id} onClick={() => setActiveSection(s.id)}
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-colors cursor-pointer flex-1 justify-center whitespace-nowrap ${activeSection === s.id ? 'bg-zinc-800 text-white' : 'text-zinc-500 hover:text-zinc-300'}`}>
