...
type TabId = 'dados' | 'empresa' | 'fiscal' | 'gestor' | 'vigencia' | 'aditivos'
           | 'documentos' | 'fiscalizacoes' | 'ocorrencias' | 'pagamentos' | 'alertas' | 'historico';
...
const TABS: Tab[] = [
  ...
  { id: 'documentos',   label: 'Documentos',    icon: <FileSignature className="h-3.5 w-3.5" /> },
      ...
      <div className="mt-6 pt-5 border-t border-zinc-800/60">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Sucessão do Contrato</span>
          </div>
          {isGestor && (
            <button onClick={() => { setEditing(true); setRefNumber(successionInfo.refNumber); setSuccType(successionInfo.type); }}
              className="text-[9px] text-zinc-500 hover:text-zinc-300 border border-zinc-800 hover:border-zinc-700 px-2 py-0.5 rounded transition-colors cursor-pointer">
              Alterar
            </button>
          )}
        </div>
        <div className="bg-violet-500/5 border border-violet-500/20 rounded-xl p-4 space-y-3">
          <p className="text-[11px] text-zinc-400 leading-relaxed">
            ...
            {' '}<span className="text-zinc-600">Registrado em {new Date(successionInfo.savedAt).toLocaleDateString('pt-BR')}.</span>
          </p>
          <button
            onClick={() => {
            ...
            <div className="flex-1 min-w-0">
              ...
              {successionInfo.label && (
                <p className="text-[10px] text-zinc-500 mt-0.5 line-clamp-1">{successionInfo.label}</p>
              )}
    ...
    return (
      <div className="mt-6 pt-5 border-t border-zinc-800/60">
        <div className="flex items-center gap-2 mb-4">
          <GitBranch className="h-3.5 w-3.5 text-violet-400" />
          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Indicar Sucessão do Contrato</span>
        </div>
        <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4 space-y-4">
          ...
          <div>
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-2">Tipo de Sucessão</p>
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                <button key={opt.key} onClick={() => { setSuccType(opt.key); setFound(null); setNotFound(false); }}
                  className={`py-2 px-3 rounded-lg text-xs font-semibold border transition-colors cursor-pointer ${succType === opt.key ? 'bg-violet-500/20 border-violet-500/40 text-violet-300' : 'bg-zinc-950 border-zinc-800 text-zinc-400 hover:border-zinc-700'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-xs text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-violet-500/50"
              />
          ...
          {found && (
            <div className="bg-emerald-500/5 border border-emerald-500/25 rounded-xl p-3 flex items-start gap-2.5">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs font-bold text-zinc-200">{succType === 'contract' ? found.contractNumber : found.processNumber}</p>
                <p className="text-[11px] text-zinc-400 line-clamp-1">{succType === 'contract' ? found.objectDescription : found.subject}</p>
                {succType === 'contract' && found.contractor && (
                  <p className="text-[10px] text-zinc-500 mt-0.5">{found.contractor?.corporateName || found.contractor?.tradeName}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-1">
            <button onClick={reset}
              className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg py-2.5 text-xs font-semibold transition-colors cursor-pointer">
              Cancelar
            </button>
    ...
    return (
      <div className="mt-6 pt-5 border-t border-zinc-800/60">
        <button onClick={() => setEditing(true)}
          className="flex items-center justify-center gap-2 w-full text-[11px] text-zinc-500 hover:text-violet-400 border border-dashed border-zinc-800 hover:border-violet-500/40 px-4 py-3 rounded-xl transition-colors cursor-pointer">
...
function GestorAlertModal({ contractId, fiscalId, fiscalName, senderName, onClose }: {
  ...
  const mutation = useMutation({
    mutationFn: () => api.alerts.sendToFiscal({ contractId, fiscalId, alertType, message }),
    onSuccess: () => {
      setSent(true);
  ...
  const selectedLabel = ALERT_TYPES.find(t => t.value === alertType)?.label || '';
      ...
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-zinc-800">
          <div>
            ...
            <p className="text-[10px] text-zinc-500 mt-0.5">Destinatário: <span className="text-zinc-300 font-semibold">{fiscalName}</span></p>
          </div>
        ...
        {sent ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm font-semibold text-white">Alerta enviado com sucesso!</p>
            <p className="text-xs text-zinc-500 mt-1">O fiscal receberá a notificação imediatamente.</p>
          </div>
        ) : (
          <div className="p-5 space-y-4">
            <div>
              <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block mb-1.5">Tipo de Comunicado</label>
              <select value={alertType} onChange={e => setAlertType(e.target.value)}
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500/50 cursor-pointer appearance-none">
                {ALERT_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest block mb-1.5">
                Mensagem <span className="text-zinc-600 normal-case font-normal">(descreva claramente a solicitação)</span>
              </label>
              <textarea value={message} onChange={e => setMessage(e.target.value)} rows={5}
                placeholder={`Ex.: Solicitamos a ${fiscalName} que encaminhe o relatório mensal...`}
                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2.5 text-xs text-white placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-blue-500/50 resize-none" />
            </div>
            <div className="bg-zinc-950/60 border border-zinc-800 rounded-xl p-3 text-[10px] text-zinc-500 space-y-0.5">
              <p><span className="text-zinc-400 font-semibold">Remetente:</span> {senderName} (Gestor de Contratos)</p>
              <p><span className="text-zinc-400 font-semibold">Tipo:</span> {selectedLabel}</p>
            </div>
            {mutation.isError && (
            ...
            <div className="flex gap-3">
              <button onClick={onClose} className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-xl py-2.5 text-xs font-semibold transition-all cursor-pointer">Cancelar</button>
...
export function ContractTabs({ contractId, user, onBack, onNavigate, onOpenMeasurement, onOpenOccurrence, onOpenAlteration, onApproveMeasurement, onRejectMeasurement, onApproveAlteration, onRejectAlteration, onResolveOccurrence, onAssignFiscal }: Props) {
  ...
  const attachDocumentMutation = useMutation({
    mutationFn: (data: any) => api.documents.attach(contractId, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['contract', contractId] }); setShowAnexarModal(false); setNewDocTitle(''); setNewDocCategory('OTHER'); },
    onError: (e: any) => alert(`Erro: ${e.message}`),
  });
  ...
  if (isLoading) return (
    <div className="h-96 bg-zinc-900/30 border border-zinc-900 rounded-xl animate-pulse" />
  );
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <button onClick={onBack} className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer">
            <ChevronLeft className="h-3.5 w-3.5" /> Contratos
          </button>
          <span className="text-zinc-700">/</span>
          <span className="text-xs font-semibold text-zinc-300">{c.contractNumber}</span>
      ...
      <div className="bg-zinc-900/30 border border-zinc-900 p-5 rounded-xl">
        <div className="flex flex-wrap justify-between items-start gap-4">
          <div className="min-w-0">
            <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">CONTRATO ADMINISTRATIVO</span>
            <h2 className="text-xl font-bold text-white mt-1">{c.contractNumber}</h2>
            <p className="text-xs text-zinc-400 mt-1 max-w-2xl">{c.objectDescription}</p>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs shrink-0">
            <div className="text-right"><span className="text-zinc-500 block text-[10px]">Valor Inicial</span><strong className="text-zinc-200">{formatCurrency(c.initialValue)}</strong></div>
            <div className="text-right"><span className="text-zinc-500 block text-[10px]">Valor Atual</span><strong className="text-emerald-400">{formatCurrency(c.currentValue)}</strong></div>
            <div className="text-right">
              <span className="text-zinc-500 block text-[10px]">Executado</span>
            ...
            <div className="text-right"><span className="text-zinc-500 block text-[10px]">{daysLeft > 0 ? 'Dias restantes' : 'Dias vencido'}</span><strong className={daysLeft <= 90 ? 'text-red-400' : daysLeft <= 180 ? 'text-amber-400' : 'text-zinc-300'}>{Math.abs(daysLeft)}</strong></div>
          </div>
        </div>
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-1 bg-zinc-900/20 border border-zinc-900 p-1.5 rounded-xl">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium transition-all cursor-pointer ${activeTab === tab.id ? 'bg-zinc-900 text-white border border-zinc-800' : 'text-zinc-500 hover:text-zinc-300'}`}>
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>
      ...
      {/* Tab content */}
      <div className="bg-zinc-900/10 border border-zinc-900 p-6 rounded-xl">
                ...
                {editingDados ? (
                  <div className="flex gap-2">
                    <button onClick={() => { setEditingDados(false); setDadosEdits({}); }}
                      className="flex items-center gap-1.5 text-[11px] bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
                  ...
                  <button onClick={() => { setEditingDados(true); setDadosEdits({}); }}
                    className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
            ...
            {editingDados ? (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                <div className="flex flex-col gap-0.5 col-span-2 md:col-span-3">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Objeto</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Unidade Administrativa</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Data de Assinatura</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Início da Vigência</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Fim da Vigência</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Valor Inicial (R$)</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Valor Atual (R$)</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Observações</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
                {(['corporateName','tradeName','cnpjCpf','email','phone','postalCode','addressStreet','addressNumber','addressNeighborhood','addressCity','addressState','stateInscription','municipalInscription'] as const).map(field => (
                  <div key={field} className="flex flex-col gap-0.5">
                    <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">{field}</span>
        ...
        {activeTab === 'fiscal' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-xs font-semibold text-zinc-300">Comissão de Fiscalização</h4>
              <div className="flex items-center gap-2">
                ...
                {isGestor && (
                  <button onClick={onAssignFiscal}
                    className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 transition-colors cursor-pointer">
            ...
            {(c.fiscalAssignments || []).length > 0 ? (
              <div className="space-y-3">
                {(c.fiscalAssignments || []).map((asg: any) => (
                  <div key={asg.id} className="bg-zinc-950/60 border border-zinc-900 p-4 rounded-xl">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center font-bold text-xs text-zinc-400">
                          {asg.fiscal?.name?.charAt(0) || '?'}
                        </div>
                        <div>
                          <p className="text-xs font-semibold text-zinc-300">{asg.fiscal?.name}</p>
                          <span className="text-[10px] text-zinc-500">{asg.fiscal?.registrationNumber} · {fiscalRoleLabel[asg.role]}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-right text-[10px] text-zinc-500">
                          <p>Portaria: <strong className="text-zinc-400">{asg.designationAct}</strong></p>
                          <p>Desde {formatDate(asg.startDate)}</p>
                          <span className={`mt-1 inline-block px-2 py-0.5 rounded border font-semibold text-[9px] ${asg.isActive ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-zinc-800 border-zinc-700 text-zinc-500'}`}>
            ...
            ) : <p className="text-xs text-zinc-500">Nenhum fiscal designado.</p>}
          </div>
        )}
        ...
        {activeTab === 'vigencia' && (
          <div className="space-y-6">
            ...
            {editingVigencia && (
              <div className="grid grid-cols-2 gap-4 bg-zinc-900/30 border border-zinc-800 rounded-xl p-4">
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Início da Vigência</span>
                ...
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Término da Vigência</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest">Início</p>
                <p className="text-sm font-bold text-zinc-200 mt-1">{formatDate(c.startDate)}</p>
              </div>
              <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest">Término</p>
                <p className="text-sm font-bold text-zinc-200 mt-1">{formatDate(c.endDate)}</p>
              </div>
              <div className={`border p-4 rounded-xl text-center ${daysLeft <= 0 ? 'bg-red-500/10 border-red-500/20' : daysLeft <= 90 ? 'bg-amber-500/10 border-amber-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest">{daysLeft > 0 ? 'Dias Restantes' : 'Dias Vencido'}</p>
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest">Prazo Decorrido</p>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-[10px] text-zinc-500">
                ...
                <span className="font-semibold text-zinc-400">{progress.toFixed(1)}% decorrido</span>
        ...
        {activeTab === 'aditivos' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h4 className="text-xs font-semibold text-zinc-300">Termos Aditivos</h4>
                <p className="text-[10px] text-zinc-500 mt-0.5">Limite: 25% (serviços/compras) · 50% (reformas)</p>
              </div>
              {(user.role === 'FISCAL' || isGestor) && (
                <button onClick={onOpenAlteration} className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
            ...
            {(c.alterations || []).length > 0 ? (
              <div className="space-y-3">
                {(c.alterations || []).map((alt: any) => (
                  <div key={alt.id} className="bg-zinc-950/60 border border-zinc-900 p-4 rounded-xl">
                    {editingAltId === alt.id ? (
                      <div className="space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                          <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Número/Identificação</span>
                          ...
                          <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Nova Data de Término</span>
                          ...
                          <div className="flex flex-col gap-0.5">
                            <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Impacto Financeiro (R$)</span>
                          ...
                          <div className="flex flex-col gap-0.5 col-span-2">
                            <span className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Justificativa</span>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => { setEditingAltId(null); setAltEdits({}); }}
                            className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 font-semibold py-1.5 rounded-lg text-xs cursor-pointer">
                            Cancelar
                          </button>
                        </div>
                      </div>
                      ...
                      <>
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-xs font-bold text-zinc-300">{alt.alterationNumber || alterationTypeLabel[alt.type]}</p>
                            <p className="text-[10px] text-zinc-500 mt-0.5">{alt.justification}</p>
                          </div>
                          <div className="flex items-center gap-2 shrink-0 ml-4">
                            ...
                            {isGestor && alt.status === 'PENDING_APPROVAL' && (
                              <div className="flex gap-1.5">
                                <button onClick={() => onApproveAlteration?.(alt.id)} className="p-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 hover:bg-emerald-500/20 cursor-pointer"><CheckCircle className="h-3.5 w-3.5" /></button>
                                <button onClick={() => onRejectAlteration?.(alt.id)} className="p-1.5 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 hover:bg-red-500/20 cursor-pointer"><XCircle className="h-3.5 w-3.5" /></button>
                              </div>
                            )}
                            {isGestor && (
                              <button onClick={() => { setEditingAltId(alt.id); setAltEdits({}); }}
                                title="Editar aditivo" className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 rounded-lg transition-colors cursor-pointer">
                        ...
                        <div className="mt-3 flex gap-4 text-[10px] text-zinc-500">
                          <span>Impacto: <strong className="text-zinc-300">{alt.valueChange >= 0 ? '+' : ''}{formatCurrency(alt.valueChange)}</strong></span>
                          {alt.newEndDate && <span>Nova vigência: <strong className="text-zinc-300">{formatDate(alt.newEndDate)}</strong></span>}
                          <span>Solicitado por: <strong className="text-zinc-400">{alt.requester?.name || alt.requestedById}</strong></span>
            ...
            ) : <p className="text-xs text-zinc-500">Nenhum aditivo registrado.</p>}
          </div>
        )}
        ...
        {activeTab === 'documentos' && (
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <h4 className="text-xs font-semibold text-zinc-300">Documentos Anexados</h4>
              {isGestor && (
                <button onClick={() => setShowAnexarModal(true)}
                  className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
            ...
            {(c.documents || []).length > 0 ? (
              <div className="space-y-2">
                {(c.documents || []).map((doc: any) => (
                  <div key={doc.id} className="bg-zinc-950/60 border border-zinc-900 p-3 rounded-xl flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <FileText className="h-5 w-5 text-emerald-400 shrink-0" />
                      <div>
                        <p className="text-xs font-semibold text-zinc-300">{doc.title}</p>
                        <p className="text-[10px] text-zinc-500">{doc.category} · {doc.fileSize > 0 ? (doc.fileSize / 1024 / 1024).toFixed(2) + ' MB' : 'Simulado'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
                      <button className="p-1.5 hover:bg-zinc-800 rounded-lg text-zinc-500 hover:text-zinc-300 transition-colors cursor-pointer">
                        <ExternalLink className="h-3.5 w-3.5" />
                      </button>
                      {isGestor && (
                        <button onClick={() => { if (confirm('Excluir este documento?')) deleteDocumentMutation.mutate(doc.id); }}
            ...
            ) : <p className="text-xs text-zinc-500">Nenhum documento anexado.</p>}

            {/* Modal Anexar */}
            {showAnexarModal && (
              <div className="fixed inset-0 bg-zinc-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl p-6 relative">
                  <button onClick={() => setShowAnexarModal(false)} className="absolute right-4 top-4 p-1 rounded-lg text-zinc-500 hover:text-white hover:bg-zinc-800 cursor-pointer"><X className="h-4 w-4" /></button>
                  <h3 className="text-sm font-bold text-white mb-4">Anexar Documento</h3>
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Título do Documento</label>
                      <input type="text" value={newDocTitle} onChange={e => setNewDocTitle(e.target.value)} className={inputCls} placeholder="Ex: Relatório de Vistoria.pdf" />
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">Categoria</label>
                      <select value={newDocCategory} onChange={e => setNewDocCategory(e.target.value)} className={inputCls}>
                        ...
                        <option value="ADDITIVE">Termo Aditivo</option>
                    ...
                    <button onClick={() => { if (!newDocTitle.trim()) return; attachDocumentMutation.mutate({ title: newDocTitle, category: newDocCategory, fileSize: 0, mimeType: 'application/pdf' }); }}
                      disabled={!newDocTitle.trim() || attachDocumentMutation.isPending}
                      className="w-full bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold py-2.5 rounded-lg text-xs cursor-pointer disabled:opacity-50">
                      {attachDocumentMutation.isPending ? 'Anexando...' : 'Confirmar Anexo'}
                    </button>
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'fiscalizacoes' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <div>
                <h4 className="text-xs font-semibold text-zinc-300">Medições de Fiscalização</h4>
                <p className="text-[10px] text-zinc-500 mt-0.5">Total medido (aprovado): {formatCurrency(totalMeasured)} de {formatCurrency(c.currentValue)}</p>
              </div>
              {user.role === 'FISCAL' && (
                <button onClick={onOpenMeasurement} className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
            ...
            {(c.measurements || []).length > 0 ? (
              <div className="space-y-3">
                {(c.measurements || []).map((msr: any) => (
                  <div key={msr.id} className="bg-zinc-950/60 border border-zinc-900 p-4 rounded-xl">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-xs font-bold text-zinc-300">{formatCurrency(msr.measurementValue)}</p>
                        <p className="text-[10px] text-zinc-500">Período: {formatDate(msr.periodStart)} — {formatDate(msr.periodEnd)}</p>
                        <p className="text-[10px] text-zinc-500 mt-0.5 line-clamp-2">{msr.reportDescription}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-4">
                        ...
                        {isGestor && msr.status === 'PENDING_GESTOR' && (
                          <div className="flex gap-1.5">
                            <button onClick={() => onApproveMeasurement?.(msr.id)} className="p-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 cursor-pointer"><CheckCircle className="h-3.5 w-3.5" /></button>
                            <button onClick={() => onRejectMeasurement?.(msr.id)} className="p-1.5 bg-red-500/10 border border-red-500/20 rounded-lg text-red-400 cursor-pointer"><XCircle className="h-3.5 w-3.5" /></button>
                          </div>
                        )}
                        {isGestor && (
                    ...
                    <div className="mt-2 flex gap-4 text-[10px] text-zinc-500">
                      <span>Fiscal: <strong className="text-zinc-400">{msr.fiscal?.name}</strong></span>
                      {msr.approver && <span>Aprovado por: <strong className="text-zinc-400">{msr.approver.name}</strong></span>}
                      <span>{formatDateTime(msr.createdAt)}</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : <p className="text-xs text-zinc-500">Nenhuma medição registrada.</p>}
          </div>
        )}
        ...
        {activeTab === 'ocorrencias' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-xs font-semibold text-zinc-300">Registro de Ocorrências</h4>
              {(user.role === 'FISCAL' || isGestor) && (
                <button onClick={onOpenOccurrence} className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
            ...
            {(c.occurrences || []).length > 0 ? (
              <div className="space-y-3">
                {(c.occurrences || []).map((occ: any) => (
                  <div key={occ.id} className="bg-zinc-950/60 border border-zinc-900 p-4 rounded-xl">
                    <div className="flex justify-between items-start">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold text-zinc-300">{occ.title}</p>
                          <p className="text-[10px] text-zinc-500 mt-0.5">{occ.description}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-4">
                        ...
                        {isGestor && occ.status === 'OPEN' && (
                          <button onClick={() => onResolveOccurrence?.(occ.id)} className="flex items-center gap-1 text-[10px] bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded-lg text-emerald-400 cursor-pointer">
                    ...
                    <div className="mt-2 flex gap-4 text-[10px] text-zinc-500">
                      <span>Registrado por: <strong className="text-zinc-400">{occ.fiscal?.name}</strong></span>
                      <span>{formatDateTime(occ.createdAt)}</span>
                      {occ.resolvedAt && <span>Resolvido em: <strong className="text-zinc-400">{formatDate(occ.resolvedAt)}</strong></span>}
                    </div>
                    {occ.resolutionDescription && (
                      <div className="mt-2 bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-2">
                        ...
                        <p className="text-[10px] text-zinc-400">{occ.resolutionDescription}</p>
                      </div>
                    )}
                  </div>
                ))}
              </div>
            ) : <p className="text-xs text-zinc-500">Nenhuma ocorrência registrada.</p>}
          </div>
        )}
        ...
        {activeTab === 'pagamentos' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-xs font-semibold text-zinc-300">Histórico de Pagamentos</h4>
              {isGestor && (
                <button onClick={() => { const v = prompt('Novo valor contratual atual (R$):', String(c.currentValue)); if (v !== null && !isNaN(Number(v))) updateDadosMutation.mutate({ currentValue: Number(v) }); }}
                  className="flex items-center gap-1.5 text-[11px] bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 px-3 py-1.5 rounded-lg text-zinc-300 cursor-pointer">
                  <Pencil className="h-3.5 w-3.5" /> Editar Valor Atual
                </button>
              )}
            </div>
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500">Total Pago (aprovado)</p>
                <p className="text-lg font-bold text-emerald-400 mt-1">{formatCurrency(totalMeasured)}</p>
              </div>
              <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500">Saldo Contratual</p>
                <p className="text-lg font-bold text-blue-400 mt-1">{formatCurrency(c.currentValue - totalMeasured)}</p>
              </div>
              <div className="bg-zinc-900/40 border border-zinc-900 p-4 rounded-xl text-center">
                <p className="text-[10px] text-zinc-500">% Executado</p>
                <p className="text-lg font-bold text-amber-400 mt-1">{executionPct}%</p>
              </div>
            </div>
            {(c.measurements || []).filter((m: any) => m.status === 'APPROVED').length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-zinc-900 text-zinc-400">
                    <th className="py-2 text-left font-semibold">Período</th>
                    <th className="py-2 text-left font-semibold">Fiscal</th>
                    <th className="py-2 text-right font-semibold">Valor</th>
                    <th className="py-2 text-left font-semibold">Aprovado em</th>
                  </tr></thead>
                  <tbody className="divide-y divide-zinc-900">
                    {(c.measurements || []).filter((m: any) => m.status === 'APPROVED').map((m: any) => (
                      <tr key={m.id} className="text-zinc-400">
                        <td className="py-2.5">{formatDate(m.periodStart)} — {formatDate(m.periodEnd)}</td>
                        <td className="py-2.5">{m.fiscal?.name}</td>
                        <td className="py-2.5 text-right font-bold text-zinc-300">{formatCurrency(m.measurementValue)}</td>
                        <td className="py-2.5">{formatDate(m.approvalDate)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            ) : <p className="text-xs text-zinc-500">Nenhum pagamento aprovado.</p>}
          </div>
        )}
...
function AlertsTab({ contractAlerts, user, queryClient }: { contractAlerts: ContractAlert[]; user: User; queryClient: any }) {
  ...
  const statusColor: Record<string, string> = {
    ...
    EXPIRED: 'text-zinc-600 bg-zinc-900 border-zinc-800',
  };
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-xs font-semibold text-zinc-300">Alertas deste Contrato</h4>
      {contractAlerts.length > 0 ? (
        contractAlerts.map(alert => (
          <div key={alert.id} className={`border p-4 rounded-xl space-y-3 ${alert.status === 'PENDING' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-zinc-950/60 border-zinc-900'}`}>
            <div className="flex justify-between items-start">
              <div className="min-w-0">
                <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-wider block">{typeLabels[alert.type] || alert.type}</span>
                <p className="text-xs font-semibold text-zinc-300 mt-0.5">{alert.title}</p>
            ...
            {alert.status === 'PENDING' && alert.type === 'CONTRACT_EXPIRING_180' && user.role === 'FISCAL' && (
              <div className="bg-zinc-900/60 border border-zinc-800 rounded-xl p-3 space-y-2">
                ...
                <p className="text-[11px] text-zinc-300">Informe ao Gestor a providência desejada para este contrato:</p>
                <div className="flex gap-2">
                  <button
            ...
            {alert.type === 'CONTRACT_EXPIRING_90' && (
              <div className="bg-red-500/5 border border-red-500/20 rounded-xl p-3">
                ...
                <p className="text-[11px] text-zinc-300 mt-1">Preparar e encaminhar o Termo Aditivo de prorrogação com urgência. Prazo crítico.</p>
              </div>
            )}

            <p className="text-[10px] text-zinc-600">{formatDateTime(alert.createdAt)}</p>
          </div>
        ))
      ) : (
...
function ContractHistory({ contractId, user }: { contractId: string; user: User }) {
  ...
  return (
    <div className="space-y-3">
      <h4 className="text-xs font-semibold text-zinc-300">Histórico de Movimentações</h4>
      {logs.length > 0 ? (
          ...
          <div className="space-y-4">
            {logs.map((log: any) => (
              <div key={log.id} className="relative pl-10">
                <div className="absolute left-2.5 top-1 h-3 w-3 rounded-full bg-zinc-700 border-2 border-zinc-900" />
                <div className="bg-zinc-950/60 border border-zinc-900 p-3 rounded-xl">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wide">{log.action}</p>
                      <p className="text-xs text-zinc-300 mt-0.5">{log.entityLabel}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0 ml-4">
                  ...
                  <p className="text-[10px] text-zinc-500 mt-1">por <strong className="text-zinc-400">{log.userName}</strong> ({log.userRole})</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <p className="text-xs text-zinc-500">Nenhuma movimentação registrada.</p>
      )}
    </div>
  );
}
