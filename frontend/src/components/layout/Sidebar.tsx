...
import { LayoutGrid, FileSignature, LogOut, Shield, FolderOpen, ShieldCheck, Users, ShieldAlert, MessageSquare, History, Brain, UserCog, Database, X, ChevronDown, Settings } from 'lucide-react';
...
function UserAvatar({ name }: { name: string }) {
  const initials = name
    ...
    : 'U';
  return (
    <div className="size-8.5 rounded-full bg-brand-blue flex items-center justify-center text-white font-bold text-xs shrink-0 shadow-[0_0_10px_rgba(20,107,255,0.4)]">
      {initials}
    </div>
  );
}
...
const roleBadgeStyle: Record<string, string> = {
  ADMIN: 'text-amber-300 bg-amber-400/10 border-amber-400/25',
  GESTOR: 'text-teal-300 bg-brand-cyan/10 border-brand-cyan/25',
  ALTA_GESTAO: 'text-violet-300 bg-violet-400/10 border-violet-400/25',
  FISCAL: 'text-emerald-300 bg-emerald-400/10 border-emerald-400/25',
};
...
export function Sidebar({ user, activeView, onNavigate, onLogout, onEditProfile, alwaysExpanded, onToggleAlwaysExpanded, collapsed, mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const renderContent = (collapsedNow: boolean) => (
    ...
    >
          ...
          {items.map(({ view, label, icon: Icon, gapAfter }) => {
            ...
            const button = (
              <button
                ...
                className={`relative flex w-full cursor-pointer items-center gap-3 rounded-xl border text-[13px] font-medium transition-all duration-150 ${
                  collapsedNow ? 'justify-center px-0 py-2.5' : 'px-3 py-2.5 text-left'
                } ${
                  active
                    ? 'border-blue-500/30 bg-[#131d2e] text-white font-semibold shadow-[0_0_12px_rgba(20,107,255,0.12)]'
              ...
              >
                ...
                {!collapsedNow && label}
                {active && <span className="absolute inset-y-2 right-0 w-[3px] rounded-l-full bg-brand-blue shadow-[0_0_8px_rgba(20,107,255,0.6)]" />}
              </button>
            );
            return (
              <div key={view}>
      ...
      <div className={`border-t border-white/[0.05] p-3 ${collapsedNow ? 'px-2' : ''}`}>
        {collapsedNow ? (
          <Tooltip content={`${user.name} · ${user.role}`} side="right">
            <button onClick={onEditProfile} className="flex w-full items-center justify-center rounded-xl border border-white/[0.04] bg-white/[0.02] p-2 transition-colors hover:border-white/10 hover:bg-white/5 cursor-pointer">
          ...
          <Dropdown
            ...
            trigger={({ open, toggle }) => (
              <button onClick={toggle} title="Menu do usuário" className="group w-full cursor-pointer">
                <div className={`flex items-center gap-3 rounded-xl border border-white/[0.05] bg-[#121720]/80 p-2.5 transition-colors hover:border-white/10 hover:bg-white/5 ${open ? 'border-white/15 bg-[#161d2a]' : ''}`}>
                  <UserAvatar name={user.name} />
        ...
        {collapsedNow ? (
          ...
          <button
            onClick={onLogout}
            className="mt-2 flex w-full cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 text-xs font-medium text-slate-400 transition-colors duration-150 hover:bg-white/5 hover:text-white"
          >
