...
let ProcessesService = class ProcessesService {
    ...
    async findOne(id, userId, role) {
        ...
        if (role === client_1.UserRole.FISCAL && process.requesterId !== userId) {
            throw new common_1.ForbiddenException('Você não tem permissão para visualizar este processo');
        }
        return process;
    }
    async create(userId, data) {
        const process = await this.prisma.procurementProcess.create({
            data: {
                ...
                status: data.status || client_1.ProcessStatus.PLANNING,
        ...
        const phases = DEFAULT_PHASES.map(ph => ({
            ...
            alertaAtivo: true,
            status: client_1.PhaseStatus.PENDING,
            isActive: true,
        }));
    ...
    async update(id, data) {
        ...
        strFields.forEach(f => { if (data[f] !== undefined)
            updateData[f] = data[f]; });
        boolFields.forEach(f => { if (data[f] !== undefined)
            updateData[f] = data[f]; });
        dateFields.forEach(f => { if (data[f] !== undefined)
    ...
    async addPhase(processId, data) {
        ...
        return this.prisma.procurementPhase.create({
            data: {
                ...
                status: data.status || client_1.PhaseStatus.PENDING,
    ...
    async updatePhase(processId, phaseId, data) {
        const phase = await this.prisma.procurementPhase.findUnique({ where: { id: phaseId } });
        if (!phase || phase.processId !== processId)
        ...
        if (data.status === 'COMPLETED' && phase.phaseNumber > 1) {
            ...
            if (previousPhase?.bloqueiaAvancoSemConclusao && previousPhase.status !== client_1.PhaseStatus.COMPLETED) {
                throw new common_1.BadRequestException(`A fase "${previousPhase.name}" precisa ser concluída antes de avançar esta fase.`);
            }
        }
        if (data.status === 'COMPLETED' && phase.phaseNumber === 6) {
            const items = (data.checklistItems || phase.checklistItems);
            const parecerItem = items?.find(i => i.id === 'c6_3');
            if (parecerItem && !parecerItem.concluido) {
                throw new common_1.BadRequestException('O Parecer Jurídico favorável deve ser marcado como concluído antes de fechar esta fase.');
            }
        }
        if (data.status === 'COMPLETED' && phase.phaseNumber === 7) {
            ...
            if (ratifItem && !ratifItem.concluido) {
                throw new common_1.BadRequestException('A ratificação/autorização assinada deve ser marcada como concluída antes de fechar esta fase.');
            }
        }
        if (data.status === 'IN_PROGRESS' && phase.phaseNumber === 13) {
            ...
            if (assinaturaPhase && assinaturaPhase.status !== client_1.PhaseStatus.COMPLETED) {
                throw new common_1.BadRequestException('O contrato deve estar assinado (fase 10 concluída) antes do início da execução.');
            }
        }
        const updateData = {};
        if (data.name)
        ...
        if (data.plannedStart !== undefined)
            updateData.plannedStart = data.plannedStart ? new Date(data.plannedStart) : null;
        ...
        if (data.pendenciaCritica !== undefined)
            updateData.pendenciaCritica = data.pendenciaCritica || null;
        if (data.checklistItems !== undefined)
            updateData.checklistItems = data.checklistItems;
        if (data.responsavelSetor !== undefined)
            updateData.responsavelSetor = data.responsavelSetor || null;
        if (data.alertaAtivo !== undefined)
            updateData.alertaAtivo = data.alertaAtivo;
